...
- A commissioner can open the shared `/join` link and QR for a limited time and optional number of requests. An owner or manager approves a pending request, then the applicant receives one activation email whose code or link verifies the address and signs them in.
