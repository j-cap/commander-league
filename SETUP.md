...
Closing the link prevents new verifications but retains already pending requests. Reopening it starts a fresh lifetime and request counter. Direct invitations remain available to the owner. Use **Einladung erneut senden** for an invited or active member when another link is needed, and **Entfernen** to permanently delete a non-owner membership, sessions and open login links.
