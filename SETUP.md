...
An owner or manager opens **Beitritt verwalten**, chooses a lifetime and optional limit, then shares `https://commanderleague.at/join` or its QR. An applicant enters a name and email address and immediately appears under **Offene Anfragen**. An owner or manager can reject the request or approve it as a member. Approval sends the only activation email; entering its six-digit code or opening its fallback link verifies the address, activates the membership and creates the six-month session. A shared link or QR never creates access and never grants manager rights. Only the owner can promote a member under **Zugänge & Rollen**.

Closing the link prevents new requests but retains already pending requests. Reopening it starts a fresh lifetime and request counter. Direct invitations remain available to the owner. Use **Einladung erneut senden** for an invited or active member when another code is needed, and **Entfernen** to permanently delete a non-owner membership, sessions and open login credentials.

The QR encodes the stable deployed join page. For a different domain, regenerate it with `python scripts/generate_qr.py https://example.test/join` (requires Python reportlab). It must never embed a login or session token.
